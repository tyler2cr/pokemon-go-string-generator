...
//   this.type = type
// }
Pokemon = (form, pokemon_id, pokemon_name, type) => {
  this.form = form;
...
const output = jsonFromApi.filter(pokemon => pokemon.form === 'Normal');

console.log("pokemon: \n", jsonFromApi.filter(pokemon => pokemon.form === 'Normal'));

const fs = require('fs');

fs.writeFile('./pokemon.json', JSON.stringify(output), err => {
  if (err) {
    console.error(err);
  }
